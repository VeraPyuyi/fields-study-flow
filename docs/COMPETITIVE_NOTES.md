...
- `report_audit.json` now also includes `experience_risks`, which checks first-screen quickstart, the first-session plan, intent-based entry choice, real Paper Map canvas affordances, branch deferral, paragraph-focused reading, local-first resources, and mobile/long-text layout safety.
