...
- `roadmap.html` resource rows now also show strength/provenance signals and their visible basis, distinguishing core resources, recommended supplements, and manual fallbacks with local availability, evidence score, trust score, recommendation score, and recommendation reasons when available.
...
- `report_audit.json` now also includes `experience_risks`, which checks first-screen quickstart, real Paper Map canvas affordances, branch deferral, paragraph-focused reading, local-first resources, and mobile/long-text layout safety.
...
- `fields-study-flow audit-report --write-release-readiness` now writes `release_readiness.md` and `release_readiness.html`, combining market readiness, visual QA, screenshot baseline status, fresh-user timing, recurring blockers, and competitor-inspired gates into a single release decision dashboard.
...
- `report_audit.json` now includes `competitive_benchmark`, which converts the competitor scan into explicit checks: PaperQA-style grounded evidence, Get It-style measurable mastery maps, roadmap.sh-style first interaction, React Flow-style canvas affordances, resource purpose badges, resource strength signals, clickable resource evidence review links, and local-first study bundles. A report cannot be marked market-ready while this benchmark warns.
...
1. Feed resource strength badges with deeper provenance, such as official-source markers and retrieval coverage across the target paper, downloaded resources, and local notes.
2. Add checked-in golden baselines for Paper Map, Paper Lens, Roadmap, and the start page, then run them in CI whenever frontend assets or report renderers change.
3. Promote release-history summaries into CI artifacts once the repository has checked-in visual baselines for the main report surfaces.
