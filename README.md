...
- Evidence coverage and mastery readiness: every export writes `evidence_coverage.md` and `evidence_coverage.html`, showing which Paper Map claims, Paper Lens paragraphs, mastery tasks, and resources are source-backed, plus clickable source diagnostics, a measured coverage score, an explain/derive/reproduce/critique readiness scorecard, and a prioritized evidence-gap queue.
