...
def test_cli_export_all_sanitizes_private_paths_and_lists_outputs(tmp_path):
    ...
    assert "evidence_coverage.html" in result.stdout
    for target in ("roadmap.json", "roadmap.md", "roadmap.svg", "index.html", "roadmap.html", "quick_brief.md", "evidence_coverage.md", "evidence_coverage.html"):
...
def test_cli_export_all_writes_paper_map_and_lens_for_paper_brief_links(tmp_path):
    ...
    assert (export_dir / "evidence_coverage.html").exists()
...
def test_write_outputs_sanitizes_private_urls_in_resource_indices(tmp_path):
    ...
    evidence_coverage_html = (tmp_path / "evidence_coverage.html").read_text(encoding="utf-8")
    ...
    assert "C:/Users/example/private" not in evidence_coverage_html
    assert "private folder" not in evidence_coverage_html
    assert "file:///C:/Users/example/private" not in evidence_coverage_html
