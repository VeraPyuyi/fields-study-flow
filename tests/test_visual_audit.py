...
ROADMAP_WORKSHEET_SYNC_MARKER = "data-roadmap-worksheet-sync data-worksheet-ready-slots data-worksheet-source-anchored mastery_worksheet.md 验收表同步 Ready-to-present"
...
def test_build_report_audit_scores_market_readiness_dimensions(tmp_path):
    for name, marker in {
        ...
        "paper_lens.html": "paragraph evidence Explanation support not a resource trust score",
            "roadmap.html": f"learning console mastery 1-minute start data-mastery-export 下载 worksheet {ROADMAP_READINESS_MARKER} {ROADMAP_WORKSHEET_SYNC_MARKER} resource-purpose-badge 为什么读 resource-strength-badge 证据强度 resource-provenance-badge resource-coverage-badge 覆盖范围 最强证据 resource-evidence-link 查看证据",
    }.items():
        (tmp_path / name).write_text(
            '<!doctype html><html><head><meta name="viewport" content="width=device-width">'
...
def test_first_session_plan_requires_panel_and_multiple_steps(tmp_path):
    for name, marker in {
        "index.html": "Start Here Bring Your Own Paper 10-minute quickstart data-market-value-panel Why this is more than a PDF summarizer intent-router-panel data-intent-router Choose by what you need data-first-session-plan data-session-step fresh-user-flow-panel data-fresh-user-flow paper_map.html paper_lens.html roadmap.html Local assets report-health-panel Report Health Evidence coverage 5 evidence-backed edges report_audit.json scenario-panel Three Learning Scenarios Single paper Paper set Field / course route data-learning-guide-panel 3 starter questions data-starter-question fields-study-flow ask data-active-recall-panel data-recall-card 5-minute active recall Find evidence",
        "paper_map.html": "Reading Density Core Chain Full Exploration Evidence coverage data-report-static-fallback=\"paper_map\" data-paper-map-canvas react-flow Download presentation notes",
        "paper_lens.html": "paragraph evidence Explanation support not a resource trust score",
        ...
        "resource_library": [
            {
                ...
                "metadata": {
                    "rag": {
                        "evidence_chunks": [
                            {"snippet": "Target-paper evidence supports this resource.", "detail_anchor": "detail-seg-1", "score": 2.4}
                        ]
                    }
                },
            }
    ...
    assert checks["first_session_plan"]["status"] == "warn"
    assert audit["market_readiness"]["status"] == "needs_improvement"
...
def test_experience_risks_require_roadmap_worksheet_sync(tmp_path):
    (tmp_path / "roadmap.html").write_text(
        '<!doctype html><html><head><meta name="viewport" content="width=device-width">'
        "<style>body{font-family:Arial;max-width:100%;overflow-wrap:anywhere}</style></head>"
        f"<body><section>{ROADMAP_READINESS_MARKER}</section></body></html>",
    ...
    assert checks["roadmap_worksheet_sync"]["status"] == "warn"

    (tmp_path / "roadmap.html").write_text(
        '<!doctype html><html><head><meta name="viewport" content="width=device-width">'
        "<style>body{font-family:Arial;max-width:100%;overflow-wrap:anywhere}</style></head>"
        f"<body><section>{ROADMAP_READINESS_MARKER} {ROADMAP_WORKSHEET_SYNC_MARKER}</section></body></html>",
        encoding="utf-8",
    )
    ...
    assert recovered_checks["roadmap_worksheet_sync"]["status"] == "pass"
...
def test_competitive_benchmark_passes_paper_centered_mastery_report(tmp_path):
    for name, marker in {
        ...
        "paper_map.html": 'Reading Density Core Chain Full Exploration Evidence coverage data-report-static-fallback="paper_map" data-paper-map-canvas react-flow Download presentation notes',
        "paper_lens.html": "paragraph evidence Explanation support not a resource trust score",
        "roadmap.html": f"learning console mastery 1-minute start data-mastery-export 下载 worksheet {ROADMAP_READINESS_MARKER} {ROADMAP_WORKSHEET_SYNC_MARKER} resource-list resource-purpose-badge 为什么读 resource-strength-badge 证据强度 resource-provenance-badge resource-coverage-badge 覆盖范围 最强证据 resource-evidence-link 查看证据 paper_lens.html#detail-seg-1",
    }.items():
        (tmp_path / name).write_text(
            '<!doctype html><html><head><meta name="viewport" content="width=device-width">'
...
def test_field_course_report_can_be_market_ready_without_paper_map_or_lens(tmp_path):
    for name, marker in {
        "index.html": _market_ready_index_marker(paper_entries=False, evidence_edges=4),
        "roadmap.html": f"learning console mastery 1-minute start data-mastery-export Download worksheet {ROADMAP_READINESS_MARKER} {ROADMAP_WORKSHEET_SYNC_MARKER} resource-list resource-purpose-badge Why read resource-strength-badge Evidence strength resource-provenance-badge resource-coverage-badge Coverage Strongest evidence resource-evidence-link Review evidence",
    }.items():
        (tmp_path / name).write_text(
            '<!doctype html><html><head><meta name="viewport" content="width=device-width">'
            "<style>body{font-family:Arial;max-width:100%;overflow-wrap:anywhere}@media (max-width: 820px){body{max-width:100%}}</style></head>"
